// =================================================================
// SCRIPT FINAL, COMPLETO E CORRIGIDO DO DASHBOARD
// =================================================================
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Inicia todas as configurações da página
    configurarNavegacao();
    ...
    // Carrega os dados do aluno do banco de dados
...
async function carregarDadosIniciais(alunoId) {
    try {
        ...
        renderizarHistorico();
        // Dispara o clique na aba de métricas para carregar a visão inicial
        document.getElementById('nav-metricas')?.click();
...
function configurarTabelasOrdenaveis() {
    document.getElementById('tabela-historico')?.querySelector('thead')?.addEventListener('click', (e) => {
        ...
        sortStateHistorico.direction = sortStateHistorico.column === header.dataset.sort && sortStateHistorico.direction === 'desc' ? 'asc' : 'desc';
        sortStateHistorico.column = header.dataset.sort;
        renderizarHistorico();
    });
    document.getElementById('container-pontos-melhorar')?.addEventListener('click', (e) => {
        const header = e.target.closest('th');
        if (!header || !header.classList.contains('sortable')) return;
        sortStateMelhorar.direction = sortStateMelhorar.column === header.dataset.sort && sortStateMelhorar.direction === 'desc' ? 'asc' : 'desc';
        sortStateMelhorar.column = header.dataset.sort;
        processarAnalisePontos();
    });
    document.getElementById('container-pontos-fortes')?.addEventListener('click', (e) => {
        const header = e.target.closest('th');
        if (!header || !header.classList.contains('sortable')) return;
        sortStateFortes.direction = sortStateFortes.column === header.dataset.sort && sortStateFortes.direction === 'desc' ? 'asc' : 'desc';
        sortStateFortes.column = header.dataset.sort;
        processarAnalisePontos();
    });
}
...
function renderizarHistorico() {
    ...
    const registrosOrdenados = [...meusRegistros].sort((a, b) => {
        let valA = a[sortStateHistorico.column]; let valB = b[sortStateHistorico.column];
        if (sortStateHistorico.column === 'desempenho') { valA = a.questoesFeitas > 0 ? (a.questoesAcertadas / a.questoesFeitas) : -1; valB = b.questoesFeitas > 0 ? (b.questoesAcertadas / b.questoesFeitas) : -1; }
    ...
    registrosOrdenados.forEach((reg) => {
        ...
        tr.innerHTML = `<td>${data}</td><td>${reg.materia || ''}</td><td>${reg.conteudo || ''}</td><td>${reg.tempoEstudado || 0}</td><td>${reg.questoesFeitas || 0}</td><td>${reg.questoesAcertadas || 0}</td><td>${desempenho}</td><td>${reg.flashcardsFeitos || 0}</td><td><div class="action-buttons"><button class="action-btn edit-btn" title="Editar">✏️</button><button class="action-btn delete-btn" title="Excluir">🗑️</button></div></td>`;
...
function showCustomModal(contentHTML) { document.getElementById('modal-body').innerHTML = contentHTML; document.getElementById('main-modal').classList.remove('hidden'); }
function hideCustomModal() { document.getElementById('main-modal').classList.add('hidden'); }
...
function filtrarRegistrosPorPeriodo(periodo) {
    ...
    switch (periodo) {
        ...
        case '12m': dataInicio = new Date(agora.getFullYear() - 1, agora.getMonth() + 1, 1); break;
...
function processarMetricas() {
    ...
    const diasAtras = 90;
    let dataInicio = new Date(); dataInicio.setDate(new Date().getDate() - diasAtras);
    for (let i = 0; i <= diasAtras; i++) {
        ...
        const diaElemento = document.createElement('div'); diaElemento.classList.add('heatmap-day');
        let nivel = 0;
    ...
    const hojeFiltro = new Date(); const seteDiasAtras = new Date(); seteDiasAtras.setDate(hojeFiltro.getDate() - 6);
    ...
    for (let i = 0; i < 7; i++) {
        ...
        dadosAcertosSemanais.push(acertosNoDia); dadosErrosSemanais.push(errosNoDia);
    }
    ...
    for (let i = diasNoFiltro - 1; i >= 0; i--) {
        const data = new Date(); data.setDate(new Date().getDate() - i);
        const dataFormatada = data.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
        labelsTempoDiario.push(dataFormatada);
        dadosTempoDiario[dataFormatada] = 0;
    }
    registrosTempoDiario.forEach(reg => { const dataFormatada = reg.dataRegistro.toDate().toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }); if (dadosTempoDiario.hasOwnProperty(dataFormatada)) { dadosTempoDiario[dataFormatada] += reg.tempoEstudado || 0; } });
...
function configurarCronometro(alunoId) {
    ...
    btnSalvar.addEventListener('click', async () => {
        ...
        const novoRegistro = {
            alunoId, materia: materiaSelecionada, conteudo: document.getElementById('cronometro-conteudo').value,

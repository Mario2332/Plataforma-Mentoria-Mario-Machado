// =================================================================
// SCRIPT DO DASHBOARD COM ABA DE HISTÓRICO E ORDENAÇÃO
// =================================================================
...
// Variáveis globais
let meusRegistros = []; // Guarda todos os registros para não precisar buscar no DB toda hora
let sortState = { column: 'dataRegistro', direction: 'desc' }; // Estado inicial da ordenação
    ...
    Object.keys(navItems).forEach(key => {
        navItems[key].addEventListener('click', () => {
            ...
            Object.keys(sections).forEach(sKey => {
                sections[sKey].classList.toggle('hidden', sKey !== key);
            });
            // Ativa/desativa menu
            Object.keys(navItems).forEach(nKey => {
                navItems[nKey].classList.toggle('active', nKey === key);
            });
            // Carrega os dados necessários para a aba
            if (key === 'metricas') carregarMetricas(sessionStorage.getItem('alunoId'));
...
function configurarCabecalhosTabela() {
    document.querySelectorAll('#tabela-historico th.sortable').forEach(header => {
        header.addEventListener('click', () => {
            ...
            renderizarHistorico(); // Re-renderiza a tabela com a nova ordenação
        });
    });
}

function renderizarHistorico() {
    // Ordena a lista de registros global
    meusRegistros.sort((a, b) => {
        let valA = a[sortState.column];
        let valB = b[sortState.column];

        // Lógica para calcular desempenho na hora, se necessário
        if (sortState.column === 'desempenho') {
            valA = a.questoesFeitas > 0 ? (a.questoesAcertadas / a.questoesFeitas) : 0;
            valB = b.questoesFeitas > 0 ? (b.questoesAcertadas / b.questoesFeitas) : 0;
        }
...
async function carregarMetricas(alunoId) {
    // Busca os dados apenas se a lista estiver vazia
    ...
    // (O restante do código de processar métricas e desenhar gráficos permanece aqui, sem alterações)
    // ...
    ...
    // ... (código dos gráficos, que continua o mesmo)
